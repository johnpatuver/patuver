...
<body>
    ...
    <header>
        ...
        <nav>
            <a href="#" onclick="showPage('painel')">Painel</a> <!-- Nova guia  Nessa guia vai ficar as configurações HEHEH -->
...
<head>
    ...
    <style>
        ...
        padding: 20px; /* Apenas para um espaçamento visual Não me amola HEHE */
    }
...
<h2>Usuários Cadastrados</h2>
<ul id="listaUsuarios">
    <!-- Os usuários cadastrados serão exibidos aqui todos vei HEHEH -->
</ul>

<script>
    // Função para exibir o pop-up ( acho os pop-up massa! )
    function showPopup() {
        var popup = document.getElementById('popup');
        popup.style.display = 'block';
        setTimeout(function() {
            popup.style.display = 'none';
        }, 2000); // Oculta o pop-up após 5 segundos  ( Achei um tempo bom, mas acha que da para deixar menor tipo 3 vamos ver mais pra frente! )
    }

    // Função para carregar os usuários cadastrados todos eles inclusive eu!  HEHEHEHH  
    ...
    // Adiciona um ouvinte de evento para o envio do formulário HEHEHEH 
    document.getElementById('cadastroForm').addEventListener('submit', function(event) {
        // Previne o envio padrão do formulário ( HEHEH )
        ...
        xhr.onreadystatechange = function() {
            if (xhr.readyState === XMLHttpRequest.DONE) {
                if (xhr.status === 200) {
                    // Se o cadastro for bem-sucedido, exibe o pop-up e recarrega os usuários ( Achei melhor trazer todos cadastrados nessa tela! )
                    showPopup();
                    carregarUsuarios();
                } else {
                    // Se houver um erro, essa e a mensagem que vai trazer de retorno truta! 
    ...
    // Carrega os usuários ao carregar a página ( Quero que seja assim, fica top HEHEH)
    carregarUsuarios();
</script>
